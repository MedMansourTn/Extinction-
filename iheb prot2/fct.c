...
struct files Initialisation(struct files fl)
{
	...
	fl.charact=IMG_Load("en1.png");
	fl.charbtn = IMG_Load("char.png");
	fl.clav=IMG_Load("clav.png");
	fl.man=IMG_Load("man.png");
	fl.posload.x=(fl.screen->w)/2-(fl.load1->w)/2;
	fl.posload.y=130;
	fl.posfullscreen.x=(fl.screen->w)/2-((fl.fullscreen1->w)/2)*2;
	fl.posfullscreen.x-=200;	
...
{	SDL_BlitSurface(f.sbg,NULL,f.screen,NULL);
	...
	while (f.y==0)
	{
		...
		f.posfullscreen.x += 230;	
		alwen(event,f.charbtn,f.charbtn,f.posfullscreen,f.screen);		
		f.posfullscreen.x += 230;	
		alwen(event,f.charbtn,f.charbtn,f.posfullscreen,f.screen);		

		f.posfullscreen.x -= 460;
		while(SDL_PollEvent(&event))
		{
			switch(event.type)
			{
				...
				{
					f.posfullscreen.x+=230;
					if ((event.button.button == SDL_BUTTON_LEFT) && pos(event,f.posfullscreen,f.charbtn))
					{
						f=charselect(f,event);
					}
					f.posfullscreen.x+=230;
					if ((event.button.button == SDL_BUTTON_LEFT) && pos(event,f.posfullscreen,f.charbtn))
					{
						f=inputselect(f,event);
					}
					f.posfullscreen.x-=460;
		                        if ((event.button.button == SDL_BUTTON_LEFT) && pos(event,f.posback,f.back1))
					{//back->menu
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
struct files charselect(struct files f,SDL_Event event)
{
	SDL_BlitSurface(f.sbg,NULL,f.screen,NULL);
	while(f.x==0)
	{			
		...
		while(SDL_PollEvent(&event))
		{
			switch(event.type)
			{
				...
				{
					/*if (pos(event,f.posquitter,f.exit1) || pos(event,f.posnewgame,f.play1) || pos(event,f.possetting,f.setting1)|| pos(event,f.posmore,f.more1))
					Mix_PlayChanne_l(-1,f.effect2,0);*/
					if (pos(event,f.posquitter,f.exit1))//PLAY-souris
						f=menu(f,event);
					
					if(pos(event,f.poschar,f.charact))
					{
						f=menu(f,event);
					}
					f.poschar.x += 100;
		...
		if (event.type==SDL_MOUSEMOTION)//ta7rik souris == verif alwen
		{
					...
					alwen(event,f.charact,f.charact,f.poschar,f.screen);
f.poschar.x += 100;
					alwen(event,f.charact,f.charact,f.poschar,f.screen);
f.poschar.x += 100;
alwen(event,f.charact,f.charact,f.poschar,f.screen);
...
struct files inputselect(struct files f,SDL_Event event)
{
	SDL_BlitSurface(f.sbg,NULL,f.screen,NULL);
	while(f.x==0)
	{			
		...
		while(SDL_PollEvent(&event))
		{
			switch(event.type)
			{
				...
				case SDL_MOUSEBUTTONDOWN:
				{
					...
					if(pos(event,f.poschar,f.man))
					{
						f=menu(f,event);
					}
					f.poschar.x += 300;

					if(pos(event,f.poschar,f.clav))
					{
						f=menu(f,event);
					}
					f.poschar.x -= 300;
		...
		if (event.type==SDL_MOUSEMOTION)//ta7rik souris == verif alwen
		{
					...
					alwen(event,f.clav,f.clav,f.poschar,f.screen);
f.poschar.x += 300;
					alwen(event,f.man,f.man,f.poschar,f.screen);
					f.poschar.x -= 300;
